        ...
        window.vapp = new Vue({
            ...
            methods: {
                ...
                getCourses: function (val) {
                    ...
                    $api.cache('Course/Pager', { 'orgid': orgid, 'sbjids': sbjid, 'thid': '', 'use': '', 'del': false, 'live': '', 'free': '', 'search': '', 'order': '', 'size': -1, 'index': 1 })
                        .then(function (req) {
                            if (req.data.success) {
                                th.courses = req.data.result;
                            } else {
                ...
                setErrorlabel: function (field) {
                    ...
                    if (label.length < 1) label = $dom('div.el-form-item__error');
                    while (label.length > 0 && label.attr('tab') == null) label = label.parent();
                    if (label.length > 0) this.activeName = label.attr('tab');
                    return false;
                },
                //提交保存操作
