...
$ready(function () {
    window.vapp = new Vue({
        ...
        data: {
            ...
            selectedparts: [],      //选择中的分类           

            rules: {
                Qp_Name: [{ required: true, message: '名称不得为空', trigger: 'blur' },
        ...
        methods: {
            ...
            getTreeData: function (orgid) {
                var th = this;
                this.loading = true;
                $api.get('ExamQues/PartTree', { orgid: orgid, search: '', isuse: null })
                    .then(function (req) {
                        if (req.data.success) {
            ...
            getEntity: function () {
                ...
                if (th.id == '' || th.id == null) {
                    $api.get('Snowflake/Generate').then(function (req) {
                        if (req.data.success) {
                            th.entity.Qp_ID = req.data.result;
                    ...
                    $api.get('ExamQues/PartForID', { 'id': th.id }).then(function (req) {
                        if (req.data.success) {
                            var result = req.data.result;
                            th.entity = result;
                            //设置当前节点禁用，防止选择自身
                            var sbj = th.traversalQuery(result.Qp_ID, th.datas);
                            if (sbj != null) sbj.Qp_IsUse = false;
                            th.traversalUse(th.datas);
                            //将当前专业的上级专业，在控件中显示
                            var arr = [];
                            arr = th.getParentPath(th.entity, th.datas, arr);
                            th.selectedparts = arr;
                        } else {
                            throw '未查询到数据';
                        }
                    }).catch(err => console.error(err))
            ...
            clone: function (entity) {
                ...
                //上级专业
                if (this.selectedparts.length > 0) obj.Qp_PID = this.selectedparts[this.selectedparts.length - 1];
                else obj.Qp_PID = 0;
                if (obj.Qp_PID == '') obj.Qp_PID = 0;
                return obj;
            },
            //操作成功
            operateSuccess: function (isclose) {
                $api.cache('ExamQues/PartForID:clear', { 'id': this.id });
                window.top.$pagebox.source.tab(window.name, 'vapp.getTreeData', isclose);
            },
            //获取当前专业的上级路径
            getParentPath: function (entity, datas, arr) {
                var obj = this.traversalQuery(entity.Qp_PID, datas);
                if (obj == null) return arr;
                arr.splice(0, 0, obj.Qp_ID);
            ...
            //所有IsUse取反，主要是Cascader控件的disabled取了IsUse作为值，它俩的意义相反
            traversalUse: function (datas, use) {
                for (let i = 0; i < datas.length; i++) {
                    const d = datas[i];
                    d.Qp_IsUse = use == null ? !d.Qp_IsUse : use;

                    if (d.children && d.children.length > 0)
                        obj = this.traversalUse(d.children, d.Qp_IsUse ? true : null);
                }
            }
        },
    });
